class Company {
    ...
    static Employee = class Employee {
        ...
        compareTo(employee) {
            let result = employee.salary - this.salary;

            return result === 0 ? this.username.localeCompare(employee.username) : result;
        }
    ...
    bestDepartment() {
        let sortedDepartments = [...this.departments]
            .sort(([aDepName, aWorkers], [bDepName, bWorkers]) => {
                return this._getAverageSalary(bDepName) - this._getAverageSalary(aDepName);
            });
...
Average salary: ${this._getAverageSalary(bestDepartmentName).toFixed(2)}`;
    ...
    _getAverageSalary(departmentName) {
        ...
        let averageSalary = departmentWorkers.reduce((acc, currWorker) => acc + currWorker.salary, 0) / departmentWorkers.length;
