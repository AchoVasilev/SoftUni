...
describe('E2E tests', function () {
    ...
    describe('Home Page [ 15 Points ]', () => {
        it('Show home page - welcome message [ 1 Points ]', async () => {
            await page.goto(host);
            await page.waitForTimeout(interval);
        ...
        it('Check home page with 0 theaters [ 3 Points ]', async () => {
        ...
        it('Check home page with 2 theaters [ 5 Points ]', async () => {
            const { get } = await handle(endpoints.catalog);
        ...
        it('Show details [ 1 Points ]', async () => {
            await page.goto(host);
            await page.waitForTimeout(interval);
        ...
        it('Check home page info [ 4 Points ]', async () => {
            const { get } = await handle(endpoints.catalog);
    ...
    describe('User Profile Page [ 10 Points ]', async () => {
        ...
        it('Check profile page for with 0 theaters [ 1.5 Points ]', async () => {
        ...
        it('Check profile page information [ 3.5 Points ]', async () => {
            const { get } = await handle(endpoints.profile(mockData.users[0]._id));
            get(mockData.catalog.slice(0, 1));
            const data = mockData.catalog.slice(0, 1);
            const user = mockData.users[0]
    ...
    describe('BONUS : Like functionality  [ 10 Points ]', async () => {
        ...
        it('Like button is visible for the non-creator user [ 2 Points ]', async () => {
            // Login user
            const user = mockData.users[0];
            const data = mockData.catalog[2];
        ...
        it('Like button is NOT visible for the creator [ 2 Points ]', async () => {
            // Login user
            const user = mockData.users[0];
            const data = mockData.catalog[0];
