...
async function getAllOrders() {
  ...
  // const url2 = `http://localhost:3030/data/orders?where=_ownerId%3D\"${userData.id}\"`;
...
async function onBuy() {
  ...
  try {
    if (furnitures.names.length == 0) {
      throw new Error('You need at least one furniture checked');
    }

    for (const name of furnitures.names) {
      name.replace('\n', '').trim();
    }
...
async function onFurnitureCreate(ev) {
  ...
  const tbody = document.querySelector('tbody');
  const response = await createFurniture(furnitureData, userData.token);
