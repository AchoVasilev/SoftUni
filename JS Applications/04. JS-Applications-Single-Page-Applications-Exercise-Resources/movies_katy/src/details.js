...
export function showDetails(movieId) { //id comes from home
    showView(section)
    getMovie(movieId)
}
...
async function getMovie(id) {
    ...
    const requests = [//returns numbers;
    ...
    const [movieData, likes, hasLiked] = await Promise.all([
        movieRes.json(),
        likesRes.json(),
        hasLikedRes && hasLikedRes.json() //shte izpulni koda, ako i dvete sa ok, t.e ako ima tretiq element i toi ne e undefined
    ]);
    
    section.replaceChildren(createDetails(movieData, likes, hasLiked));
}

export function createDetails(movie, likes, hasLiked) {
